...
4. **TTS Synthesis**: Use Irodori-TTS with ADR-0020 stability protocol. If `reference.wav` is present in the asset directory, reference-based synthesis is automatically enabled and validated against ECAPA-TDNN speaker similarity (threshold >= 0.80) with automatic retries on drift.
