import { IdentityContractSchema, type IdentityContract, type EmotionalState, type SceneState } from "../runtime/types";
...
export class IdentityManager {
  ...
  load(contractData: any): IdentityContract {
    ...
    console.log(`[IDENTITY] Loaded contract: ${this.activeContract.name} (${this.activeContract.id})`);
  ...
  validateInvariants(
    ...
    // Delta checks
    if (lastEmotion) {
      const eDelta = Math.abs(emotion.arousal - lastEmotion.arousal) + 
