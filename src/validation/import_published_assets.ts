...
for (const dir of directories) {
	...
	for (const contractFile of contractFiles) {
		...
		try {
			...
			// パブリッシュに成功しているか確認
			if (
				contract.verification?.status !== "PASS" ||
				!contract.verification?.remote_proof?.videoId
			) {
				console.log(
					`⚠️ パブリッシュ未完了またはFAILセッションのためインポートをスキップします: ${contract.sessionId}`,
				);
				continue;
			}
			...
			const primitives = situation.map((segment: any, idx: number) => {
				...
				let archetype:
					...
					| "proximity_shift"
					| "tactile_reassurance"
					| "silence_architecture" = "silence_architecture";
			...
			const exclusivityCount = (
				rawTextFull.match(/私だけ|わたしだけ|二人きり|キミには私/g) || []
			).length;
			...
			const scriptAuditData: ASMRScriptAudit = {
				...
				acoustic: {
					...
					panning_cue_count:
						contract.verification.evidence.runtime_traces.chunk_count,
			...
			console.log(
				`🎉 データベースへの永続化に成功しました！ (ScriptID: ${scriptId})`,
			);
			importCount++;
		} catch (err: any) {
...
console.log(`\n=================================================`);
console.log(
	`インポート処理完了: 合計 ${importCount} 件のアセットを HCI-DB 2.0 に登録しました。`,
);
console.log(`=================================================`);
