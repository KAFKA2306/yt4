...
import { AuditTrace, ProductionConfig, ScriptLine } from "./types";
	...
	async run(config: ProductionConfig) {
		...
		for (let i = 0; i < chunks.length; i++) {
			...
			while (attempt < config.runtime.max_retries && !passed) {
				const p = path.join(this.assetDir, `${config.identity.id}_${sessionId}_p${i}_v${attempt}.wav`);
				const caption = generateCaption(config.identity.voice_id, currentChunk[0].emotion as any) + (currentOverrides.caption_suffix || "");

				await synthesizeVoice({
					text: currentChunk.map(l => l.text).join(" "),
					caption,
					outputPath: p,
					seed: config.runtime.seed_base + i + (currentOverrides.seed_offset || 0),
					// temperature: currentOverrides.temperature // Need to support in tts.ts
				});
				...
				const trace: AuditTrace = {
					...
					promptHash: AuditLogger.calculateHash(currentChunk.map(l => l.text).join(" ")),
				...
				if (judgment.status === "PASS") {
					...
					if (judgment.repair_candidate) {
						const repairResult = this.repair.apply(currentChunk, judgment.repair_candidate, attempt + 1);
			...
			if (!passed) {
				throw new Error(`CRITICAL: Chunk ${i} failed after ${config.runtime.max_retries} attempts. Pipeline TERMINATED.`);
			}
		}
